...
{
    QString filePath = QFileDialog::getSaveFileName(0, tr("选择文件存储路劲"), "" , "*.txt");
    if (filePath == NULL)
    {
        return;
    }
...
{
    packthead->start();
    connect(packthead, SIGNAL(cpDateDumpIsNullSignal()), this, SLOT(cpDateDumpIsNullSlot()));
}
...
bool OpenLog::startDateDump()  //打包主流程
{
    ...
    if (latestdate != NULL)
    {
        QString filePath = QFileDialog::getSaveFileName(0, tr("选择文件存储路劲"), "" , "*.zip");
        if (filePath == NULL)
        {
            return false;
        }
